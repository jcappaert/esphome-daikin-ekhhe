    ...
    cv.Schema(
        {
            ...
            cv.Optional(P53_EVA_FAN_DEFR_SPEED): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=100): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=0): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P54_LOW_PRESS_BYPASS): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=240): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
                cv.Optional(CONF_DEVICE_CLASS, default=DEVICE_CLASS_DURATION): cv.string,
                cv.Optional(CONF_UNIT_OF_MEASUREMENT, default=UNIT_MINUTE): cv.string,
            }),
            cv.Optional(P55_EVA_BAND1_PROP): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=20): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P56_EVA_MAX_ACT_DELTA): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=20): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P57_EVA_MAX_DEACT_DELTA): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=20): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P60_EVA_AIR_DELTA1): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=25): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P61_EVA_AIR_DELTA2): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=25): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P62_EVA_AIR_DELTA3): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=25): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P63_EVA_AIR_DELTA4): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=25): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P64_EVA_AIR_DELTA5): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=25): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P65_EVA_AIR_DELTA6): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=25): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P66_EVA_BAND2_PROP): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=20): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P67_EVA_BAND3_PROP): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=20): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P68_EVA_BAND4_PROP): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=20): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P69_EVA_BAND5_PROP): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=20): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P70_EVA_BAND6_PROP): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=20): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P71_EC_FAN_SILENT_RED): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=40): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=0): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
            ...
            cv.Optional(P72_EC_FAN_REG_GAIN): number.number_schema(DaikinEkhheNumber).extend({
                cv.GenerateID(): cv.declare_id(DaikinEkhheNumber),
                cv.Optional(CONF_MAX_VALUE, default=100): cv.float_,
                cv.Optional(CONF_MIN_VALUE, default=1): cv.float_,
                cv.Optional(CONF_STEP, default=1): cv.positive_float,
